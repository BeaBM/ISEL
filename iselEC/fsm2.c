...
#define PRECIOCAFE	50
  ...
  void  digitalWrite(int a, int b){
      printf("digitalWrite: %d valor %d", a,b);
    }
...
{
  time_t timerid;// cambiado de timer_t
  struct spec value;
  struct sigevent se;
...
static int button_pressed (fsm_t* this)
{
  int ret = button;
    ...
    //else //if(cuenta>=PRECIOCAFE)&&(ret==1){       
       //  flagBoton=1;
    //}
  return ret;
}
...
{
    ///problema al meter monedas sin aretar boton
     if((cuenta >= PRECIOCAFE) && (flagBoton==1)){
         flagBoton=0;
...
//  digitalWrite (GPIO_LED, HIGH);
  ...
  while (1) {
      //añadir cosas para activar cafe en funcion de mnedero
